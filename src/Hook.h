...
///ＨＯＯＫ的目的就是以同步地的写法来写异步调用
///一般异步调用写法　＝　异步调用　＋　回调函数　＝＝＞　asyncCall() { data -> callback(data)　}
///通过ＨＯＯＫ之后，可以用同步的方式去写异步调用
///比如：　 auto data = asyncCall();
///       callback(data);　这里看是同步调用的写法，但是其实还是异步调用
namespace Server {
    bool is_hook_enable();
...
///extern:在C语言中，修饰符extern用在变量或者函数的声明前，
